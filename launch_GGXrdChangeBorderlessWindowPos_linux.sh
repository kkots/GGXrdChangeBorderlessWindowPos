#!/usr/bin/env bash
# Make sure to give yourself permission to launch this script with 'chmod u+x launch_GGXrdChangeBorderlessWindowPos_linux.sh' command.
# Then cd into the same directory as this script and launch it using './launch_GGXrdChangeBorderlessWindowPos_linux.sh' or 'bash launch_GGXrdChangeBorderlessWindowPos_linux.sh'.
...
TOOLNAME32="${1-GGXrdChangeBorderlessWindowPos.exe}"  # ${VARNAME-DEFAULTVALUE} substitutes DEFAULTVALUE in case the variable is empty
TOOLNAME64="${1-GGXrdChangeBorderlessWindowPos64bit.exe}"
