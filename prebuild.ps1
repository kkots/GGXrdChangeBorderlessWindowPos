...
function replace_about_dlg($content_lines) {
	...
	for ($i = 0; $i -lt $content_lines.Count; ++$i) {
		...
		if ($line -match "\s*IDD_ABOUTBOX DIALOGEX") {
			$inside_dialog = $true;
		} elseif ($inside_dialog -and ($line -match ("\s*LTEXT\s+`"\S+\s+Version\s+[^`"\s]+`""))) {
			$pos1 = $line.IndexOf("`"");
			++$pos1;  # skip quote
			# skip project name
...
$content = Get-Content "GGXrdChangeBorderlessWindowPos.rc"
$newline = "`
"
$tab = "	";
...
if ($need_write) {
	Set-Content -Value ($content_lines -join $newline) "GGXrdChangeBorderlessWindowPos.rc" -Encoding Unicode
}
